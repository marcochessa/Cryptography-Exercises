 ...
 *
 * ****************************TIPS***********************
 * TLS uses asymmetric cryptography for securely generating and exchanging a session key.
 * The session key is then used for encrypting the data transmitted by one party,
...
#define ENCRYPT 1
#define DECRYPT 0
#define MAX_ENC_LEN 1048576 // 1 MB
...
int main()
{
    ...
    /* Generate key and IV random of 128 bit */
    ...
    while ((n_read = fread(buffer, 1, MAX_BUFFER, file_in)) > 0)
    {
        ...
        { // use EVP_CIPHER_get_block_size with OpenSSL 3.0+
    ...
    unsigned char encrypted_data[MAX];
